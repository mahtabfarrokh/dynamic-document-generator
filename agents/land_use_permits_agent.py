...
class LandUsePermitsContent(BaseModel):
    html_section: str  # Full styled HTML block

async def generate_land_use_permits(data: DocState, mock_response=None) -> str:
    print("starting land use permits generation")
    response = await acompletion(
        ...
        messages=[
            ...
            },
            {
                "role": "user",
                "content": (
                    "Generate the Land Use Permits section content. "
                    "Make it realistic, clear, and professional. Keep it concise."
                )
            }
        ],
        temperature=0.2,
        response_format=LandUsePermitsContent, 
    ...
    try:
        validated = LandUsePermitsContent.parse_raw(json_content)
        print("finished land use permits generation")
